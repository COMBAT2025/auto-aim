...
Calibrator::Calibrator(Setter::CalibratorPara &temp,MindVisionCamera &tempCamera
)//传入的参数是setter中的地址和工业相机的驱动

{
    const char* image_folders= temp.SAVE_IMAGE_PATH.c_str();
    cv::Size board_size=temp.CornersMatrix;
    cv::Size square_size=temp.Rectangle;
  auto_Start(image_folders,tempCamera,board_size,square_size);
    std::vector<cv::String>filename;
    cv::glob(image_folders, filename);
    cv::Size image_size_temp;
    std::cout << "begin to add points..." << std::endl;
    addChessBoardCorners(filename, board_size, square_size, image_size_temp);
    save_calibrate_result(temp.CalibrationResultPath);//cr
    for (int i = 0; i < filename.size(); i++) {
        saveImagePoints(temp.CalibrationPointPath, image_points[i], object_points[i]);//ip
        number++;
    }
...
void Calibrator::auto_Start(
   ...
   // MindVisionCamera industryCamera2;
    ...
    while (1) {
        ...
        cv::flip(src,src,-1);
        timeCount++;
        if (40 == timeCount && i < 25) {
            //循环25次，每隔1.5秒自动拍一次照片，保存至规定文件夹
            if (0 != cv::findChessboardCorners(src, board_size, image_corners))
            {
                i++;
